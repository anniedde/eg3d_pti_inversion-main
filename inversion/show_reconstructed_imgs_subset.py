...
if args.pti:
    out_dir = f'out/reconstructions_subset_pti'
else:
    out_dir = f'out/reconstructions_subset_no_pti'
...
for fileName in image_paths:
    if fileName.endswith('.png'):
        ...
        for i, model_name in enumerate(models_list):
            if args.pti:
                img_loc = os.path.join('embeddings', 'luchao_subset_test_imgs', model_name, id_name, 'final_rgb_proj.png')
            else:
                img_loc = os.path.join('embeddings', 'luchao_subset_test_imgs', model_name, id_name, 'before_pti_rgb_proj.png')
            img = transform(PIL.Image.open(img_loc).convert('RGB'))
            imgs.append(img)
            print('i = ', i, ' , img_count = ', img_count)
