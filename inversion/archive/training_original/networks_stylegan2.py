...
class SynthesisLayerWrapper(torch.nn.Module, LoRALayer):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        x = modulated_conv2d(x=x, weight=(self.synthesis_layer.weight + (self.lora_B @ self.lora_A).view(self.synthesis_layer.weight.shape) * self.scaling), styles=styles, noise=noise, up=self.synthesis_layer.up,
            padding=self.synthesis_layer.padding, resample_filter=self.synthesis_layer.resample_filter, flip_weight=flip_weight, fused_modconv=fused_modconv)
...
class AdaSynthesisLayerWrapper(torch.nn.Module, LoRALayer):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        flip_weight = (self.synthesis_layer.up == 1) # slightly faster
        x = modulated_conv2d(x=x, weight=(self.synthesis_layer.weight + (self.lora_B @ (self.lora_A * self.lora_E)).view(self.synthesis_layer.weight.shape) * self.scaling / (self.ranknum+1e-5)), styles=styles, noise=noise, up=self.synthesis_layer.up,
            padding=self.synthesis_layer.padding, resample_filter=self.synthesis_layer.resample_filter, flip_weight=flip_weight, fused_modconv=fused_modconv)
