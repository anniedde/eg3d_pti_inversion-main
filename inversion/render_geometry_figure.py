...
def render(fname, size, sigma_threshold, w_frames, outdir, id_name, model_name):
    ...
    with open(f'embeddings/orange_cat_original/{model_name}/IMG_9909/optimized_noise_dict.pickle', 'rb') as f:
            optimized_noise_dict = pickle.load(f)
    
    device = torch.device('cuda')
    ws = torch.from_numpy(optimized_noise_dict['projected_w']).to(device)
    with dnnlib.util.open_url(f'networks/orange_cat/{model_name}.pkl') as f:
        G = legacy.load_network_pkl(f)['G_ema'].to(device) # type: ignore
    G.load_state_dict(torch.load(f'embeddings/orange_cat_original/{model_name}/IMG_9909/tuned_G.pt'))
    for i in tqdm(range(num)):
        ...
        intrinsics = torch.tensor([[4.2647, 0, 0.5], [0, 4.2647, 0.5], [0, 0, 1]]) # pixel unit
        ...
        img2 = (img * 127.5 + 128).clamp(0, 255).to(torch.uint8)[0]
        img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        
        
        images_out_interpolate.append(img)
        print('shape of img: ', np.array(img2).shape)
        #video_writer.append_data(np.array(img2).astype(np.uint8))
    video_writer.close()
    ...
    for j, image in enumerate(images_out):
        Image.fromarray(image[0].cpu().numpy(), 'RGB').save(os.path.join('cat_figure', 'pose{}.png'.format(j)))


    # put input image at the beginning
    input_img = PIL.Image.open(os.path.join('/playpen-nas-ssd/awang/data/orange_cat_test', f'{id_name}.png')).convert('RGB')
